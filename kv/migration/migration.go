...
func ErrInvalidMigration(n string) error {
	return fmt.Errorf(`DB contains record of unknown migration %q - if you are downgrading from a more recent version of influxdb, please run the "influxd downgrade" command from that version to revert your metadata to be compatible with this version prior to starting influxd.`, n)
}
...
func (m *Migrator) walk(ctx context.Context, store kv.Store, fn func(id platform.ID, m Migration)) error {
	if err := store.View(ctx, func(tx kv.Tx) error {
		...
		return kv.WalkCursor(ctx, cursor, func(k, v []byte) (bool, error) {
			...
			idx := int(id) - 1
			if idx >= len(m.Specs) {
				return false, ErrInvalidMigration(mig.Name)
			}
